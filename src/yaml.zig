...
fn parseStruct(self: Yaml, arena: Allocator, comptime T: type, map: Map) Error!T {
    ...
    inline for (struct_info.fields) |field| {
        const value: ?Value = map.get(field.name) orelse blk: {
            const field_name = try std.mem.replaceOwned(u8, arena, field.name, "_", "-");
...
pub fn toString(self: Yaml, writer: anytype) !void {
...
const supportedNullValue: [4][]const u8 = .{ "~", "null", "Null", "NULL" };
...
const longestBooleanValueString = blk: {
    ...
    break :blk std.mem.max(usize, &lengths);
};

const longestNullValueString = blk: {
    var max_len: usize = 0;
        ...
        switch (self) {
            ...
            .map => |map| {
                ...
                for (map.keys(), map.values()) |key, value| {
                    ...
                    if (should_inline) {
                        try value.toString(writer, args);
                    } else {
                        try writer.writeByte('\n');
